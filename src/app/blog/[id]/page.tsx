...
export default function SingleBlogPost() {
  ...
  return (
    <>
      <Header />
      <div className="min-h-screen bg-gradient-to-r from-[#0a0829] to-[#050219]">
        {/* Article Container */}
        <article className="max-w-5xl 2xl:max-w-7xl mx-auto px-4 py-16">
          {/* Article Header */}
          <header className="mb-12">
            <div className="flex items-center gap-4 mb-6">
              <span className={`px-4 py-2 text-xs font-britanica rounded-full bg-gradient-to-r bg-[#FFB600] text-black shadow-md`}>
                {post.category}
              </span>
              <time className="text-sm text-white font-britanica-regular">{post.date}</time>
            </div>
            <h1 className="text-4xl md:text-5xl font-bold font-britanica mb-6 leading-tight bg-clip-text text-transparent bg-gradient-to-r from-[#FFB600] to-[#3703FF]">
              {post.title}
            </h1>
          </header>
          ...
          <div className="prose prose-lg max-w-none">
            {post.content.map((paragraph, index) => {
              ...
              return (
                <p key={index} className="mb-6 text-white leading-relaxed font-britanica-regular">
                  {paragraph}
                </p>
              );
            })}
            ...
            {showFlags && (
              <div className="my-12">
                <h3 className="text-2xl font-britanica mb-6 text-white">Qualified Nations</h3>
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                  {countries.map((country, idx) => (
                    <div key={idx} className="flex items-center p-4 bg-[#FFB600] rounded-lg shadow-md hover:shadow-lg transition-shadow">
          ...
          <div className="mt-16">
            <Link
              href="/blog"
              className="inline-flex items-center font-britanica gap-2 px-6 py-3 text-white rounded-lg hover:shadow-lg transition-all"
            >
          ...
          <section className="mt-24 border-t border-[#FFB600] pt-12">
            <h2 className="text-2xl font-britanica mb-8 text-white">More from our blog</h2>
            <div className="grid md:grid-cols-3 gap-8">
              {blogPosts
                .filter(p => p.id !== id)
                .slice(0, 3)
                .map(relatedPost => (
                  <article key={relatedPost.id} className="group bg-[#050219] rounded-xl overflow-hidden shadow-md hover:shadow-xl transition-all">
                    <Link href={`/blog/${relatedPost.id}`}>
                      ...
                      <div className="p-6">
                        <span className="inline-block px-3 py-1 text-xs font-britanica rounded-full bg-[#FFB600] text-white mb-3">
                          {relatedPost.category}
                        </span>
                        <h3 className="text-xl text-white font-britanica mt-1 mb-2 group-hover:text-[#3703FF] transition-colors">
                          {relatedPost.title}
                        </h3>
